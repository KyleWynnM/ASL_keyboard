...
model = load("full_alphabet_scaled_ratio_based_svm_hand_gesture_classifier.joblib")
...
while cap.isOpened():
    ...
    cv2.putText(frame, f"Typed Text: {typed_text}", (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    ...
    if key == ord(' '):  # Space bar is pressed
        typed_text += gesture  # Add the current gesture to the typed text

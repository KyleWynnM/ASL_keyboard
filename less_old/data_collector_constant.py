...
# Define a function to calculate ratios between landmarks
def calculate_ratios(landmarks):
    ratios = []
    for i in range(len(landmarks)):
        for j in range(i+1, len(landmarks)):
            # Calculate the ratios between landmark coordinates
            ratio_x = landmarks[i].x / landmarks[j].x
            ratio_y = landmarks[i].y / landmarks[j].y
            ratio_z = landmarks[i].z / landmarks[j].z
            ratios.extend([ratio_x, ratio_y, ratio_z])
    return ratios

# Define a function to write ratios and key to a CSV file
